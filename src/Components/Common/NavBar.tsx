...
interface MenuItemProps {
    ...
    isExpand: boolean;
    isSelecetd: boolean,
    updateMenuBarState: (toggleExpand: boolean, menu: string | null) => void;
}
        ...
        <>
            {
                ...
                <div className="bar text-white" style={barStyle}>
                    <div className="border-bottom border-top border-white d-flex flex-row align-items-end justify-content-center" style={{ height: '10vh' }}>
                        {!isMobileView && <img src={pocket} alt="" style={MenuIcon} onClick={() => updateMenuBarState(true, null)} />}
                        {menuBarState.isExpanded && <span style={MenuNameStyle}>Gadgets</span>}
                    </div>

                    <div style={{ height: "60vh", marginTop: "2vh" }}>
                        {[
                            { name: "Dashboard", icon: "bi-pip" },
                            { name: "Chat with me", icon: "bi-robot" },
                            { name: "To do List", icon: "bi-journal-bookmark" },
                            { name: "Gym", icon: "bi-person-walking" },
                            { name: "Expense tracker", icon: "bi-wallet2" }
                        ].map(item => (
                            <MenuItem
                                key={item.name}
                                icon={item.icon}
                    ...
                    <MenuItem icon={"bi-gear-fill"} name={"Setting"} isExpand={menuBarState.isExpanded} isSelecetd={menuBarState.selectedMenu === "Setting"} updateMenuBarState={updateMenuBarState} />

                    <div className="d-flex flex-row align-items-center justify-content-center border-top border-white" style={{ height: '10vh' }}
                        onClick={() => updateMenuBarState(true, null)}>
